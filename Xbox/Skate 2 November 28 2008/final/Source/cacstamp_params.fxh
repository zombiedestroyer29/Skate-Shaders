...
sampler i_decal = sampler_state { MinFilter = ANISOTROPIC; MagFilter = ANISOTROPIC; Mipfilter = LINEAR; MaxAnisotropy = 4; TrilinearThreshold = THREEEIGHTHS; };
sampler i_decal2 = sampler_state { MinFilter = ANISOTROPIC; MagFilter = ANISOTROPIC; Mipfilter = LINEAR; MaxAnisotropy = 4; TrilinearThreshold = THREEEIGHTHS; };
sampler i_alpha;
float4 i_boneArray [ 61*3 ];
float4 i_decal1UV [ 3 ];
float4 i_decal2UV [ 3 ];
float4 i_enableDecal2 [ 3 ];
float4 i_modulateColor1;
float4x4 i_matWorld;
float4 i_shmult [ 9 ];
