...
half fShadowVal = CSM_ShadowMap_Self_PS ( float4 ( In.vPos, 1 ) , g_vViewPos , 1.0 , half3 ( g_charattributes[0].x, 0.8117647, 0.3882353 ) );
...
half alphaValue = 0.0;
half4 bruiseEnabled = half4 ( 0.0, 0.0, 0.0, 0.0 );

half4 finalBruiseCoords = half4 ( 0.25, 0.25, 0.25, 0.0 );
half finalColorVal = 0.0f;
...
bruisePart = i_decal2UV[2];
alphaValue = clrAlpha.b;
bruiseEnabled = i_enableDecal2[2];
}

if ( alphaValue > 0.8 ) {

finalBruiseCoords.z *= bruisePart.x;
finalColorVal = bruiseEnabled.x;
} else 
if ( alphaValue > 0.6 ) {

finalBruiseCoords.z *= bruisePart.y;
finalColorVal = bruiseEnabled.y;
} else 
if ( alphaValue > 0.4 ) {

finalBruiseCoords.z *= bruisePart.z;
finalColorVal = bruiseEnabled.z;
} else 
 if ( alphaValue > 0.2 ) {

finalBruiseCoords.z *= bruisePart.w;
finalColorVal = bruiseEnabled.w;
}

clrDecal1Stamp = tex2D ( i_decal , In.UV.zw*finalBruiseCoords.xy+finalBruiseCoords.zw );

clrDecal1Stamp.a *= finalColorVal;


cDiffuse = cDiffuse*( 1.0-clrDecal1Stamp.a )+clrDecal1Stamp*clrDecal1Stamp.a;
}
