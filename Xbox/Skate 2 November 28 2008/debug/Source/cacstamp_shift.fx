...
cDiffuse.a = 1.0;
half fShadowVal = CSM_ShadowMap_Self_PS ( float4 ( In.vPos, 1 ) , g_vViewPos , dot ( g_vLightDir.xyz , wNormal.xyz ) , g_charattributes[0].w );
half fWorldShadowVal = CalculateWorldShadow ( float4 ( In.vPos, 1 ) );

half clrAlpha = tex2D ( i_alpha , In.UV.xy ).r;

float4 clrDecal1Stamp = half4 ( 0.0, 0.0, 0.0, 0.0 );
float4 normalizedBUV = half4 ( 0.0, 0.0, 0.0, 0.0 );

 if ( In.UV.z > 0.0 &amp;&amp; In.UV.w > 0.0 &amp;&amp; In.UV.z < 1.0 &amp;&amp; In.UV.w < 1.0 ) {

clrDecal1Stamp = tex2D ( i_decal , In.UV.zw );
cDiffuse = cDiffuse*( 1.0-clrDecal1Stamp.a )+clrDecal1Stamp*clrDecal1Stamp.a;
}

half4 clrDirt = tex2D ( i_decal2 , In.UV.xy );

cDiffuse = cDiffuse*( 1.0-clrDirt.a )+clrDirt*clrDirt.a;
